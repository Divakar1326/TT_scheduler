...
class RuleValidator:
    ...
    def validate_entities(self, parameter: Dict[str, Any]) -> List[str]:
        ...
        if room_no and str(room_no).upper() not in ("ANY", "ROOM", "ROOMS", "ALL", "ALL_ROOMS") and not self.room_repo.find_by_no(room_no):
            errors.append(f"Room {room_no} does not exist.")
            
        lab_room_no = parameter.get("lab_room_no")
        if lab_room_no and str(lab_room_no).upper() not in ("ANY", "LAB", "LABS", "ALL", "ALL_LABS") and not self.lab_repo.find_by_no(lab_room_no):
